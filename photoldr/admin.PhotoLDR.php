...
$sqlb=$res41[0]-10;
$sql3 = "select * from ".$dbprefix."banners ORDER BY id LIMIT $sqlb,10";
...
/*echo "<pre>"; print_r($res31);
echo "helooqwer";
*/
 $sqla=$res31[0]-10;
/*$afs=mysql_query($sqla);echo "hello";
print_r($afs);echo "hello12";*/
...
$track2->addChild('option',"article:Hidden:posturl:http://www.example.com/?q=photoldrpost.php");
...
$track2->addChild('option',"page:Hidden:posturl:http://www.example.com/?q=photoldrpost.php");
...
//echo $value;
 ...
 $track4->addAttribute('count', $count3);
   ...
   if($title[$j] == ''){}else{
   $track5->addChild('title1',"$title[$j]");
  }
  $track5->addChild('log');
 if($state[$j] == ''){}else{
  ...
  $track5 = $track4->addChild('node');
   ...
   if($name1[$j] == ''){}else{
   $track5->addChild('title1',"$name1[$j]");
  }
  $track5->addChild('log');
  ...
  for($j = 0;$j<=$count4-1;$j++){
   ...
   if($title1[$j] == ''){}else{
   $track5->addChild('title1',"$title1[$j]");
  }
  $track5->addChild('log');
